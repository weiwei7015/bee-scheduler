...
public class TaskDao extends DaoBase {
    ...
    public Pageable<Task> query(String name, String group, String state, int page) {
        ...
        StringBuilder sqlQueryResultCount = new StringBuilder("SELECT COUNT(1) FROM QRTZ_TRIGGERS t1 JOIN qrtz_job_details t2 ON t1.JOB_NAME = t2.JOB_NAME AND t1.JOB_GROUP = t2.JOB_GROUP JOIN qrtz_cron_triggers t3 ON t1.TRIGGER_NAME = t3.TRIGGER_NAME AND t1.TRIGGER_GROUP = t3.TRIGGER_GROUP");
        StringBuilder sqlQueryResult = new StringBuilder("SELECT t1.TRIGGER_NAME 'name',t1.TRIGGER_GROUP 'group',t1.TRIGGER_TYPE 'triggerType',t2.JOB_CLASS_NAME 'jobComponent',t3.CRON_EXPRESSION 'cron',t1.JOB_DATA 'data',t1.TRIGGER_STATE 'state',t1.PREV_FIRE_TIME 'prevFireTime',t1.NEXT_FIRE_TIME 'nextFireTime',t1.START_TIME 'startTime',t1.END_TIME 'endTime',t1.MISFIRE_INSTR 'misfireInstr',t1.DESCRIPTION 'description' FROM QRTZ_TRIGGERS t1 JOIN qrtz_job_details t2 ON t1.JOB_NAME = t2.JOB_NAME AND t1.JOB_GROUP = t2.JOB_GROUP JOIN qrtz_cron_triggers t3 ON t1.TRIGGER_NAME = t3.TRIGGER_NAME AND t1.TRIGGER_GROUP = t3.TRIGGER_GROUP");
    ...
    public Task get(String name, String group) {
        String sql = "SELECT t1.TRIGGER_NAME 'name',t1.TRIGGER_GROUP 'group',t2.JOB_CLASS_NAME 'jobComponent',t3.CRON_EXPRESSION 'cron',t1.JOB_DATA 'data',t1.DESCRIPTION 'description' FROM QRTZ_TRIGGERS t1 JOIN qrtz_job_details t2 ON t1.JOB_NAME = t2.JOB_NAME AND t1.JOB_GROUP = t2.JOB_GROUP JOIN qrtz_cron_triggers t3 ON t1.TRIGGER_NAME = t3.TRIGGER_NAME AND t1.TRIGGER_GROUP = t3.TRIGGER_GROUP where t1.SCHED_NAME = ? and t1.TRIGGER_NAME = ? and t1.TRIGGER_GROUP = ?";
